...
% maximum sizer size
-define(OuterSizerMaxHeight, 420).
...
make_window(Acceptors, Proposers, AccPanelHeight, PropPanelHeight) ->
    ...
    ProposerSizer = wxStaticBoxSizer:new(
        ?wxVERTICAL,
        Panel,
        [{label, "Proposers"}]
    ),
    AcceptorSizer = wxStaticBoxSizer:new(
        ?wxVERTICAL,
        Panel,
        [{label, "Acceptors"}]
    ),
...
create_acceptors(AcceptorList, Panel, AcceptorSizer, Env) ->
    AcceptorData = lists:map(
        fun(AccTitle) ->
            AcceptorSizerIn = wxStaticBoxSizer:new(
                ?wxVERTICAL,
                Panel,
                [{label, AccTitle}]
            ),
    ...
    lists:map(
        fun({AcceptorPanel, AcceptorSizerIn, Lb1, Lb2}) ->
            spawn(fun() ->
                wx:set_env(Env),
...
create_proposers(ProposerList, Panel, ProposerSizer, Env) ->
    ProposerData = lists:map(
        fun({PropTitle, TextColour}) ->
            ProposerSizerIn = wxStaticBoxSizer:new(
                ?wxVERTICAL,
                Panel,
                [{label, PropTitle}]
            ),
    ...
    lists:map(
        fun({ProposerPanel, ProposerSizerIn, Lb1}) ->
            spawn(fun() ->
                wx:set_env(Env),
